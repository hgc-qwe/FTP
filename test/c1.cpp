...
void Download(int fd) {
    ...
    cout << "remote file path: ";
    cin >> remote_path;

    cout << "save as(local filename): ";
    cin >> local_file;
    ...
    SendCommand(fd, "RETR " + remote_path);
    string resp = ReadResponse(fd);
    cout << resp;
...
void Upload(int fd) {
    ...
    long long offset = GetFileSize(fd, remote_path);
    ...
    SendCommand(fd, "STOR " + remote_path);
    string resp = ReadResponse(fd);
    cout << resp;
    if (resp.find("150") == string::npos) {
        fclose(fp);
