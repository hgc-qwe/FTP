...
void ProcessCommand(int client_fd, const string& cmd, bool& quit, int& data_listen_fd, bool& logged_in, string& username) {
    ...
    if (op == "USER") {
        ...
        } else if (username == "hgc-qwe" && arg == "1234567") {
            logged_in = true;
        ...
        if (getcwd(cwd, sizeof(cwd)) != nullptr) {
            SendResponse(client_fd, "257 \"" + string(cwd) + "\" is current directory.");
        } else {
            SendResponse(client_fd, "550 Failed to get directory.");
        }
    }
    else if (op == "SIZE") {
        if (!logged_in) {
        ...
        if (chdir(arg.c_str()) == 0) {
            ...
            SendResponse(client_fd, "550 Failed to change directory.");
        }
    }
    else {
...
void ReceiveClientData(int client_fd) {
    ...
    while (!quit) {
        ...
        while ((pos = line_buf.find("\r\n")) != string::npos) {
            ...
            ProcessCommand(client_fd, cmd, quit, data_listen_fd, logged_in, username);
            if (quit) break;
        }
    }
